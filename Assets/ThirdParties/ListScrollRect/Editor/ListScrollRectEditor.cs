...
public class ListScrollRectEditor : Editor
{
	...
	private void DrawListScrollRect()
	{
		...
		{
			EditorGUILayout.HelpBox("Values not editable in play mode. If you would like to make changes to them while the game is running use the RebuildContent or RefreshContent method.", UnityEditor.MessageType.Info);
	...
	private void DrawContentFillerInterface()
	{
		...
		{
			EditorGUILayout.HelpBox("GameObject does not have an attached component that implements the IContentFiller interface.", UnityEditor.MessageType.Error);
		}
	}
