...
public class ListScrollRect : UnityEngine.UI.ScrollRect
{
    ...
    private bool _isInitializeButDisable;

    private System.Action _delayedRebuildContent;
	...
	protected virtual void Update()
	{
        ...
        if(_delayedRebuildContent != null)
        {
            _delayedRebuildContent();
            _delayedRebuildContent = null;
    ...
    protected override void OnEnable()
    {
        ...
        if(_isInitializeButDisable && !Initialized)
        {
            _isInitializeButDisable = false;
            Initialize();
        }
    }

#if UNITY_EDITOR && !JENKINS_AUTOBUILD
	protected override void OnValidate()
	{
		...
		if (content != null && !Application.isPlaying)
		{
            ...
            //Comment this code because setting Archor.Min/Max in OnValidate will cause SendMessage Error
        ...
        if(!initialized)
        {
            if(_initCoroutine == null)
            {
                if(gameObject.activeInHierarchy)
                    _initCoroutine = StartCoroutine(DoInitialize());
                else
                    _isInitializeButDisable = true;
            }
            return false;
	...
	public void RebuildContent()
	{
        ...
        {
            _delayedRebuildContent = RebuildContent;
#if UNITY_EDITOR
            Debug.LogWarning("Rebuild Content in hidden state? postpone the rebuild action");
#endif
        }
        else
		{
	...
	public void GoToListItem(int itemIndex)
	{
        ...
        if( itemIndex < 0 )
        {
            ...
            Debug.LogWarning( string.Format( "[ListScrollRect] GoToListItem: itemIndex ({0}) must be greater than of equal to 0.", itemIndex ) );
            return;
        }
        else if( itemCount == 0 )
        {
        ...
        else if( itemIndex >= itemCount )
        {
            ...
            Debug.LogWarning( string.Format( "[ListScrollRect] GoToListItem: itemIndex ({0}) must be less than the item count ({1}).", itemIndex, itemCount ) );
            return;
        }
    ...
    private static int LIMIT_REFRESH_INDEX = 0;
	...
	{
        LIMIT_REFRESH_INDEX++;
        if(LIMIT_REFRESH_INDEX > MAX_LIMIT)
        {
            //執行太多次了 直接中斷吧
            Debug.LogWarning("List Scroll Rect => RefreshContent Too Much!!!!");
            LIMIT_REFRESH_INDEX = 0;
            return;
        }
	...
	private bool UpdateListItems()
	{
		...
		if (lowestVal >= bottomPosValue && listInfo.CurrentAvgSize > 0)
		{
		...
		if (highestVal <= topPosValue && listInfo.CurrentAvgSize > 0)
		{
			// Calculate how many items we think will go in that gap
			float	gapSize			= topPosValue - highestVal;
			int		numItemsInGap	= listInfo.CurrentAvgSize > 0 ? Mathf.CeilToInt(gapSize / listInfo.CurrentAvgSize) : 0;
