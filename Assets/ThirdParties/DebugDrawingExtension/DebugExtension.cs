...
public static class DebugExtension
{
	...
	/// </param>
	/// <param name='up'>
	/// 	- The up perpendicular to the surface of the circle.
	/// </param>
	/// <param name='color'>
	...
	/// 	- The up for the cone gets wider in.
	/// </param>
	/// <param name='angle'>
	...
	/// </param>
	/// <param name='direction'>
	/// 	- The up the arrow will point in.
	/// </param>
	/// <param name='color'>
	...
	/// </param>
	/// <param name='direction'>
	/// 	- The up for the cone to get wider in.
	/// </param>
	/// <param name='color'>
	/// 	- The color of the cone.
	/// </param>
    ...
    public static void DrawSphere( Vector3 debugWireSpherePosition, Color color, float debugWireSphereRadius )
    {
        Color originalColor = Gizmos.color;

        Gizmos.color = color;
        Gizmos.DrawWireSphere( debugWireSpherePosition, debugWireSphereRadius );
        Gizmos.color = originalColor;
    }

    public static void DrawSector( Vector3 position, Color color, float startAngle, float angle, float radius, Vector3 up )
    {
        up = ( up == Vector3.zero ? Vector3.up : up ).normalized * radius;
        Vector3 forward = Vector3.Slerp( up, -up, 0.5f );
        Vector3 right = Vector3.Cross( up, forward ).normalized * radius;
        ...
        Vector3 lastPoint = position + matrix.MultiplyPoint3x4( new Vector3( Mathf.Cos( (-90- startAngle ) * Mathf.Deg2Rad ), 0, Mathf.Sin( ( -90 - startAngle ) * Mathf.Deg2Rad ) ) );
        ...
        for( var i = 0 ; i < 91 && drawAngle <= angle ; i++ )
        {
            drawAngle += 4;

            nextPoint.x = Mathf.Cos( ( - 90 - startAngle - i * 4 ) * Mathf.Deg2Rad );
            nextPoint.z = Mathf.Sin( ( - 90 - startAngle - i * 4 ) * Mathf.Deg2Rad );
            nextPoint.y = 0;
    ...
    public static void DebugSector( Vector3 position, Color color, float startAngle, float endAngle, float radius, Vector3 up )
    {
        up = ( up == Vector3.zero ? Vector3.up : up ).normalized * radius;
        Vector3 forward = Vector3.Slerp( up, -up, 0.5f );
        Vector3 right = Vector3.Cross( up, forward ).normalized * radius;
        ...
        Debug.DrawLine( position, lastPoint, color );

        int drawAngle = 0;

        for( var i = 0 ; i < 91 && drawAngle <= endAngle ; i++ )
        {
            drawAngle += 4;
        ...
        Debug.DrawLine( lastPoint, position, color );
    }
}
